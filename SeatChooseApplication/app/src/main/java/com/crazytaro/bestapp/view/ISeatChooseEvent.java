...
public interface ISeatChooseEvent {
     ...
     * <p><font color="yellow"><b>此方法实际上是对{@link #seatSeleted(int, int, int)}方法的简单处理并返回结果</b></font>，
     * 使用此方法请注意如果所有参数使用默认值则没有问题，如果座位类型有进行了自定义或者是全部替换默认的座位类型，
     * 请注意是否有重新设置座位参数中的默认座位处理类型{@link com.crazytaro.bestapp.draw.utils.SeatParams#setSeatTypeConstant(int, int, int)},
     * 此方法的简单处理依赖于上述方法中设置的静态变量{@link com.crazytaro.bestapp.draw.utils.SeatParams#SEAT_TYPE_SELETED}用于判断当前座位是否被选中</p>
     *
    ...
    /**
     * 座位选中回调接口
     * <p><font color="yellow"><b>此方法处理的情况及可选范围较大，如果处理比较复杂的座位类型建议使用此方法,此方法必定会被调用（除非没单击到有效区域） </b></font></p>
     * <p>当选中的位置出错或不存在座位时，返回座位错误类型{@link com.crazytaro.bestapp.draw.utils.SeatParams#SEAT_TYPE_ERRO}</p>
     *
     * @param rowIndex    座位在列表中的行索引
     * @param columnIndex 列索引
