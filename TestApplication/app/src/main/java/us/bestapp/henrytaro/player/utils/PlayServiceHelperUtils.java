...
public class PlayServiceHelperUtils implements MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnBufferingUpdateListener {
    ...
    private us.bestapp.henrytaro.player.utils.UpdateSeekBarUtils mUpdateSeekbarUtils;
    ...
    public PlayServiceHelperUtils(Activity uiActivity, SeekBar updateSeekBar) {
        ...
        mUpdateSeekbarUtils = us.bestapp.henrytaro.player.utils.UpdateSeekBarUtils.getInstance();
    }
    ...
    public void initial(ServiceConnection connection) {
        ...
        //创建服务
        Intent serviceIntent = new Intent(mApplicationContext, us.bestapp.henrytaro.player.service.PlaySerive.class);
        //启动服务
    ...
    public void onResume(Activity uiActivity, SeekBar updateSeekbar) {
        us.bestapp.henrytaro.player.interfaces.ITrackHandleBinder binder = us.bestapp.henrytaro.player.service.PlaySerive.getBinder();
        if (binder != null) {
     ...
     * @param binder
     */
    public void onServiceConnect(us.bestapp.henrytaro.player.interfaces.ITrackHandleBinder binder) {
        if (binder != null) {
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        //若程序不在前台运行,停止更新进度条
        if (us.bestapp.henrytaro.player.utils.CommonUtils.isBackground(mApplicationContext)) {
            mUpdateSeekbarUtils.pauseUpdate();
        } else {
            //尝试更新进度条
