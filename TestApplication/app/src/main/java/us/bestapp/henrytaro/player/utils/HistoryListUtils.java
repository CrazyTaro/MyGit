...
import us.bestapp.henrytaro.player.interfaces.IHistoryListHandle;
...
public class HistoryListUtils implements IHistoryListHandle {
    ...
    private List<AbsTrack> mHistoryList;
    //历史列表上限,小于0代表无上限,大于0代表上限值为该值
    private int mHistoryUpperLimit = -1;
    //更新历史列表时的回调接口
    private List<IHistoryLisUpdateListener> mHistoryUpdateCallbackList;
    //当前播放历史列表的使用的索引
    ...
    public boolean isPlayingHistoryTrackByPrevious(AbsTrack currentTrack) {
        if (mCurrentHistoryIndex < 0) {
            return true;
        } else if (mCurrentHistoryIndex == 0 && mHistoryList.size() > 0) {
            return mHistoryList.get(0).equals(currentTrack);
        } else {
            return false;
        }
    }
     ...
     */
    public void recycle() {
        mHistoryUpdateCallbackList.clear();
        mHistoryUpdateCallbackList = null;
        mHistoryList.clear();
        mHistoryList = null;
    }
...
//            throw new RuntimeException("play list can not be null");
     ...
     * @return
     */
    public boolean addLastPlayTrack(AbsTrack track) {
        if (track == null) {
            return false;
        }
        //检查列表是否存在
        ...
        //判断当前列表中是否存在此歌曲,是则移除
        if (mHistoryList.contains(track)) {
            this.removeTrack(track, STATE_REMOVE_ADD);
        } else if (isHistoryListFull()) {
            this.removeTrack(0, STATE_REMOVE_ADD);
        }
        //添加歌曲并回调事件
        this.addTrack(track);
        return true;
     ...
     * @return
     */
    public AbsTrack getHistoryPlayTrack(AbsTrack currentTrack) {
        //判断当前的历史列表索引是否合法
        if (mCurrentHistoryIndex >= mHistoryList.size()) {
            return null;
        }
        //获取当前历史索引的歌曲
        AbsTrack currentHistory = mHistoryList.get(mCurrentHistoryIndex);
        //进行歌曲的匹配确定歌曲是否与当前历史索引记录歌曲相同(即是否在播放历史列表中的歌曲)
        if (currentHistory.equals(currentTrack)) {
            //若是尝试获取再上一首的历史歌曲
     ...
     * 获取最后一次播放过的歌曲
     */
    public AbsTrack getLastPlayTrack() {
        if (!checkHistoryLisExsit()) {
            return null;
        }
     ...
     */
    private void addTrack(AbsTrack track) {
        mHistoryListLock.lock();
        mHistoryList.add(0, track);
        mHistoryListLock.unlock();
        if (mHistoryUpdateCallbackList.size() > 0) {
            for (IHistoryLisUpdateListener listener : mHistoryUpdateCallbackList) {
                listener.onAddHistoryTrack(track);
            }
        }
    }
     ...
     */
    private void removeTrack(AbsTrack track, String removeState) {
        mHistoryListLock.lock();
        boolean isSuccess = mHistoryList.remove(track);
        mHistoryListLock.unlock();
        if (mHistoryUpdateCallbackList.size() > 0) {
            for (IHistoryLisUpdateListener listener : mHistoryUpdateCallbackList) {
                listener.onRemoveHistoryTrack(track, removeState);
            }
        }
    }
     ...
     * @param removeState 表示当前移除歌曲事件发生的位置,提供更精确的处理判断,{@link #STATE_REMOVE_ADD},{@link #STATE_REMOVE_UPDATE}
     */
    private void removeTrack(int index, String removeState) {
        AbsTrack oldTrack = mHistoryList.get(index);
        removeTrack(oldTrack, removeState);
    }
        ...
        if (checkHistoryLisExsit()) {
            if (mHistoryList.size() > limit) {
                ...
                for (int i = 0; i < offset; i++) {
                    removeTrack(i, STATE_REMOVE_UPDATE);
                }
            }
        }
    }
